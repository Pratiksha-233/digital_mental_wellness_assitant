...
class StressCalculationService:
    ...
    # Emotion stress weights (how much each emotion contributes to stress)
    ...
    def calculate_stress_level(self, user_id: int) -> dict:
        ...
        stress_level = (
            emotion_score * 0.35 +      # 35% weight on emotions
            mood_score * 0.25 +         # 25% weight on mood
            energy_score * 0.15 +       # 15% weight on energy
            activity_score * 0.15 +     # 15% weight on activity
            trend_score * 0.10          # 10% weight on trends
        )
    ...
    def _get_emotion_stress_score(self, user_id: int) -> float:
        ...
        try:
            ...
            # Calculate average stress from emotions
            stress_scores = []
            for row in emotions:
            ...
            # Also include face emotion if available
            cursor.execute("""
    ...
    def _get_energy_stress_score(self, user_id: int) -> float:
        ...
        try:
            ...
            # Get average energy level from recent mood logs
            cursor.execute("""
            ...
            # Scale: 10 = 100 stress, 0 = 0 stress (inverted)
            # Lower energy = higher stress
    ...
    def _get_activity_stress_score(self, user_id: int) -> float:
        ...
        try:
            ...
            # Get activity frequency (last 7 days)
            cursor.execute("""
            ...
            # Healthy baseline: ~50 activities per week (~7 per day)
            # Below baseline = increased stress
            baseline = 50
            if activity_count >= baseline:
                activity_stress = 20  # Good activity level
            elif activity_count >= baseline * 0.5:
                activity_stress = 50  # Moderate
            else:
                activity_stress = 80  # Low activity = high stress
    ...
    def _get_primary_emotion(self, user_id: int) -> str:
        ...
        try:
            ...
            # Get most common emotion from last 7 days
            cursor.execute("""
    ...
    def _get_mood_trend(self, user_id: int) -> str:
        ...
        try:
            ...
            # Get average energy for this week vs last week
            cursor.execute("""
                SELECT 
                    (SELECT AVG(energy_level) FROM mood_logs 
    ...
    def _get_stress_recommendations(self, stress_category: str, 
        ...
        # Add emotion-specific recommendation if available
